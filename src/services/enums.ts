...
export async function GetAlertTypes() {
  const res = await httpClient.private.get<MainResponse<string[]>>(
    "/enums/getNotificationType",
  );
  return res.data;
}
...
export async function GetContaminationCovTypes() {
  const res = await httpClient.private.get<MainResponse<string[]>>(
    "/enums/getContaminationCoverageTypes",
  );
  return res.data;
}
...
export async function GetSurfaceCondition() {
  const res = await httpClient.private.get<MainResponse<string[]>>(
    "/enums/getSurfaceCondition",
  );
  return res.data;
